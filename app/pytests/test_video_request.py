...
class SimpleTest(TestCase):
    ...
    def test_send_video_request(self):
        try:
            ...
            Notification.objects.create(user=sender, message=f'You have sent a video request to '+ str(receiver) +'.', type=3, videoRequest_id=recentVideoRequest)  # create notification for sender
            Notification.objects.create(user=receiver, message=f'You have received a video request from '+ str(sender) +'.', type=4,videoRequest_id=recentVideoRequest) # create notificaiton for receiver
